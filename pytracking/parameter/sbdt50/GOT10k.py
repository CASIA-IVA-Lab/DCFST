...
def parameters(ID=None):
    ...
    params.learning_rate = 0.02          # Learning rate to update locator
    ...
    params.target_not_found = 0.2        # Absolute score threshold to detect target missing 
    ...
    params.hard_negative_learning_rate = 0.1     # Learning rate if hard negative samples are detected (small)
    ...
    params.window_sigma_factor = 1.0    # Standard deviation of Gaussian window relative to target size (large)
    ...
    params.scale_damp = 0.4              # Linear interpolation coefficient for target scale update (small)
