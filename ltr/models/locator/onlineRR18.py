...
class OnlineRRNet(nn.Module):
    ...
    def get_locator_feat(self, feat, proposals):
        ...
        # Convolution
        feat_layer_3 = self.conv3_2(self.conv3_1(feat[0]))
        feat_layer_4 = self.conv4_2(self.conv4_1(feat[1]))
        ...
        feat_layer_3_fc_norm = feat_layer_3_fc / ((torch.sum(feat_layer_3_fc.abs()**2, dim=1, keepdim=True) / (feat_layer_3_fc.shape[1] + 1e-10))**(1/2))
        feat_layer_4_fc_norm = feat_layer_4_fc / ((torch.sum(feat_layer_4_fc.abs()**2, dim=1, keepdim=True) / (feat_layer_4_fc.shape[1] + 1e-10))**(1/2))
